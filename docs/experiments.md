...
| 2026-05-09 | C++ port skeleton: binary pair index loader | Python build script gets `--write-bin` that emits a flat zlib-free format alongside `.npz`: 72-byte header (magic / version / counts / bin params), then raw `vectors` / `magnitudes` / `bin_keys` / `bin_offsets` / `pair_endpoints` arrays, then a length-prefixed `star_ids` blob. New C++ loader `astro_navigation::localization::load_pair_index_bin` populates a `PairIndex` struct. `apps/lost_in_space_pair_id` CLI loads the index and prints a JSON summary matching the Python build. | 500-star fixture: stars=500 / bins=2394 / pairs=101,124, bin_arcsec=120, edges 0.2/80 — bit-exact match between C++ summary and Python JSON. 16000 mag&le;8 fixture: stars=16000 / bins=2395 / pairs=88,885,824, `.bin` size 712 MB (uncompressed, vs `.npz` 292 MB compressed). C++ loads without issue. | First C++ deliverable in the LIS port. Subsequent sessions add `candidate_mappings`, `verify_rotation`, and the pyramid+restart loop in Eigen, matching the Python output bit-exactly on the same fixtures |
| 2026-05-09 | TRN cycle 4: terminal-descent extension on real LRO/LOLA | Same `scripts/lro_trn_demo.py`, finer LRO data: WAC z=8 (~82 m/px, 9-25 tiles ~350 KB-1 MB total) + `LDEM_64` (~470 m/px, ~530 MB one-time download). Tile_radius increased to 2 (5x5 = ~92 km × 92 km mosaic at z=8) so the rover footprint at 30 km altitude (~57 km) fits with margin. | **Best per-target altitude (z=8, LDEM_64, alt sweep 20/30/50/100 km)**: tycho 32 m @ 30 km (matches=82, inliers=11, median reproj 0.10 px); copernicus 29 m @ 50 km; apollo17 43 m @ 30 km; apollo15 130 m @ 50 km; apollo12 93 m @ 100 km; apollo11 172 m @ 100 km. **Median ~80 m on a ~92 km mosaic, ~10x tighter than orbital cycle 3 (179-1383 m at 400 km)**. **Cliff at < 30 km altitude**: detailed PnP diagnostic on Tycho z=8 LDEM_64 alt 30 km shows 96 putative matches but only **2 / 96 true inliers under truth pose** (reprojection < 5 px). Source is parallax distortion from real heightmap: Tycho rim at +1.8 km vs camera at 30 km altitude → ~6 % image-position shift between top-down ortho view and rover view; SIFT scale-space and Lowe ratio cannot bridge that geometric mismatch. | TRN third axis is now operationally useful at terminal-descent altitudes (30-100 km), not just orbital. Honest cliff at < 30 km is a 3D-aware-matching problem; ASIFT (affine SIFT) or rendering the ortho through the same heightmap projection before matching are the two viable next-cycle approaches. apollo11/12 (mare) need ~3x higher altitude than crater rims because dark mare features are dim and self-similar at z=8 sample distance. Output at `docs/figures/trn_lro_tycho_terminal/{ortho,rover,matches}.png + summary.json`. |
